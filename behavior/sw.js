/* service worker ของเกม "สถานีวิจัยพฤติกรรมสัตว์" (พฤติกรรมของสัตว์ ม.6)
...
self.addEventListener('activate', e=>{
  e.waitUntil((async ()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('behavior-') && k!==CACHE).map(k=>caches.delete(k)));
